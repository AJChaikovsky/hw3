...
Circuit::Circuit() : m_pq(2, EventLess()) {
...
void Circuit::parse(const string& filename) {
  ...
  for(int i = 0; i < numInject; i++) {
    ...
    char state = tokens[2][0]; //first character of the state string      
    Event* e = new Event();
    e->time = time;
    e->wireId = wireId;
    e->state = state;
...
void Circuit::run() {
  while (!m_pq.empty()) {
    ...
    while(!m_pq.empty() && m_pq.top()->time == currentTime){
      Event* e = m_pq.top();
      m_pq.pop();
      //find affected wire and update its state
